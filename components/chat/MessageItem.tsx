...
import { dataUrlToBlob, getBlobForRef, isBlobRef, isImageValue, useBlobRefUrl } from '../../utils/blobRef';
...
/** 聊天图片：轻点看大图，保存按钮走手机系统分享/保存。 */
const ChatImagePreview: React.FC<{
    value: string;
    className?: string;
    alt?: string;
    ...
    onLoad?: () => void;
}> = ({ value, className, alt = '聊天图片', loading, decoding, onLoad }) => {
    ...
    const handleSave = async () => {
        ...
        setSaveState('idle');
        try {
            const blob = isBlobRef(value)
                ? await getBlobForRef(value)
                : value.startsWith('data:')
                    ? dataUrlToBlob(value)
                    : await fetchBlobForShare(src, 'image/png');
            if (!blob) throw new Error('图片已失效');
            const extension = (blob.type.split('/')[1] || 'png').replace(/[^a-z0-9]/gi, '') || 'png';
            await shareOrDownloadBlob({ blob, fileName: `聊天图片-${Date.now()}.${extension}`, shareTitle: '保存聊天图片' });
            setSaveState('done');
        } catch (error) {
            console.warn('[ChatImage] save failed:', error);
            setSaveState('error');
        } finally {
            setSaving(false);
        }
    ...
    return <>
        <button type="button" className="group/image relative block max-w-full cursor-zoom-in rounded-2xl text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400" onClick={() => setExpanded(true)} aria-label="点击放大图片">
            <img src={src} className={className} alt={alt} loading={loading} decoding={decoding} onLoad={onLoad} />
            <span className="pointer-events-none absolute bottom-2 right-2 rounded-full bg-slate-950/55 px-2 py-1 text-[10px] font-medium text-white opacity-0 transition-opacity group-hover/image:opacity-100">点击放大</span>
        </button>
        {expanded && typeof document !== 'undefined' && createPortal(<div className="fixed inset-0 z-[999] overflow-hidden bg-slate-950/95" style={{ paddingTop: 'var(--safe-top)', paddingBottom: 'var(--safe-bottom)' }} role="dialog" aria-modal="true" aria-label="查看聊天图片" onClick={() => setExpanded(false)}>
            <div className="absolute inset-0 flex items-center justify-center p-2" onClick={event => event.stopPropagation()}>
                <img src={src} className="h-full w-full rounded-xl object-contain shadow-2xl" alt={alt} />
            </div>
            <div className="absolute left-4 top-3 z-10 text-xs font-medium text-white/70">聊天图片</div>
            <button type="button" className="absolute right-4 top-3 z-10 rounded-full bg-white/15 px-3 py-1.5 text-xs font-bold text-white hover:bg-white/25 focus:outline-none focus-visible:ring-2 focus-visible:ring-white" onClick={() => setExpanded(false)}>关闭</button>
            <div className="absolute bottom-4 right-4 z-10" style={{ marginBottom: 'var(--safe-bottom)' }}>
                <button type="button" onClick={event => { event.stopPropagation(); void handleSave(); }} disabled={saving} className="rounded-full bg-white px-5 py-2.5 text-xs font-bold text-slate-800 shadow-lg transition-transform active:scale-95 disabled:opacity-60 focus:outline-none focus-visible:ring-2 focus-visible:ring-white">
                    {saving ? '保存中…' : saveState === 'done' ? '已保存' : saveState === 'error' ? '保存失败，重试' : '保存到手机'}
                </button>
            </div>
        </div>, document.body)}
    </>;
...
interface MessageItemProps {
    ...
    onResolveLifeRecord?: (m: Message, action: 'confirmed' | 'rejected') => void;
    /** 用户点角色提出的日程建议卡 → 确认写入 / 拒绝。 */
    onResolveCalendarProposal?: (m: Message, action: 'accepted' | 'rejected') => void;
    /** 用户点角色赠送的星光纪念品 → 收下 / 拒绝。 */
    onResolveStarlightGift?: (m: Message, action: 'accepted' | 'rejected') => void;
...
const StarlightGiftBubble: React.FC<{ metadata: any; message: Message; onResolve?: (m: Message, action: 'accepted' | 'rejected') => void }> = ({ metadata, message, onResolve }) => {
    const [imageUrl, setImageUrl] = useState<string | null>(null);
    useEffect(() => { let url: string | null = null; if (!metadata?.previewImageRef && metadata?.giftId) GalleryStore.list().then(items => { const item = items.find(entry => entry.id === metadata.giftId); if (item) { url = URL.createObjectURL(item.imageBlob); setImageUrl(url); } }).catch(() => {}); return () => { if (url) URL.revokeObjectURL(url); }; }, [metadata?.giftId, metadata?.previewImageRef, metadata?.status]);
    const pending = metadata?.status === 'pending';
    const rejected = metadata?.status === 'rejected';
    const previewImage = typeof metadata?.previewImageRef === 'string' && metadata.previewImageRef ? metadata.previewImageRef : imageUrl;
    return <div className="w-[min(280px,72vw)] overflow-hidden rounded-3xl border border-white/70 bg-white/60 p-3 shadow-lg backdrop-blur-xl"><div className="flex items-center gap-2 text-sm font-black text-indigo-700">🎁 {metadata?.title || '神秘礼物'}</div>{!rejected && previewImage ? <ChatImagePreview value={previewImage} className="mt-3 h-36 w-full rounded-2xl object-cover" alt={metadata?.title || '礼物'} /> : <div className="mt-3 flex h-20 items-center justify-center rounded-2xl bg-indigo-100/70 px-3 text-center text-xs text-indigo-500">{pending ? (metadata?.previewError || '正在准备这份纪念品…') : rejected ? '你拒绝了这份纪念品' : '纪念品已收进馆中'}</div>}<p className="mt-3 text-xs leading-relaxed text-slate-600">{metadata?.message || '这是一份藏在星光里的心意。'}</p>{pending ? <div className="mt-3 flex gap-2"><button type="button" className="flex-1 rounded-xl bg-indigo-500 px-3 py-2 text-xs font-medium text-white" disabled={!onResolve} onClick={() => onResolve?.(message, 'accepted')}>收下并入馆</button><button type="button" className="flex-1 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-medium text-slate-600" disabled={!onResolve} onClick={() => onResolve?.(message, 'rejected')}>拒绝</button></div> : <div className={`mt-2 text-center text-[10px] font-bold ${rejected ? 'text-slate-400' : 'text-pink-500'}`}>{rejected ? '已拒绝，不会进入纪念馆' : '✨ 已收进星光纪念馆'}</div>}</div>;
};
...
const MessageItem = React.memo(({
    ...
    onResolveTransfer,
    onResolveLifeRecord,
    onResolveCalendarProposal,
    onResolveStarlightGift,
    ...
    if (isSystem) {
        ...
        const calendarProposal = m.metadata?.calendarEventProposal as {
            title?: string;
            date?: string;
            type?: string;
        ...
        if (calendarProposal) {
            const status = calendarProposal.status || 'pending';
            return (
                <div className={`flex items-center w-full ${selectionMode ? 'pl-8' : ''} animate-fade-in relative transition-[padding] duration-300`}>
                    ...
                    <div className="w-full px-4 my-3" {...interactionProps}>
                        <div className="mx-auto w-full max-w-sm rounded-2xl border border-indigo-100 bg-white/90 p-4 shadow-sm">
                            <div className="flex items-start gap-3">
                                <div className="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-indigo-50 text-indigo-500">日</div>
                                <div className="min-w-0 flex-1">
                                    <div className="text-xs font-semibold text-slate-700">角色申请添加一条日程</div>
                                    <div className="mt-1 truncate text-sm font-medium text-slate-900">{calendarProposal.title || '未命名事件'}</div>
                                    <div className="mt-0.5 text-xs text-slate-500">{calendarProposal.date} · {calendarProposal.type || 'TODO'}</div>
                                </div>
                            </div>
                            {status === 'pending' ? (
                                <div className="mt-3 flex gap-2">
                                    <button type="button" className="flex-1 rounded-xl bg-indigo-500 px-3 py-2 text-xs font-medium text-white transition-colors hover:bg-indigo-600 disabled:opacity-50" disabled={!onResolveCalendarProposal} onClick={() => onResolveCalendarProposal?.(m, 'accepted')}>确认添加</button>
                                    <button type="button" className="flex-1 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-medium text-slate-600 transition-colors hover:bg-slate-50 disabled:opacity-50" disabled={!onResolveCalendarProposal} onClick={() => onResolveCalendarProposal?.(m, 'rejected')}>拒绝</button>
                                </div>
                            ) : (
                                <div className={`mt-3 rounded-xl px-3 py-2 text-center text-xs ${status === 'accepted' ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-500'}`}>
                                    {status === 'accepted' ? '已添加到日程' : '已拒绝添加'}
                                </div>
                            )}
                        </div>
                    </div>
    ...
    if (m.metadata?.starlightGift) return commonLayout(<StarlightGiftBubble metadata={m.metadata} message={m} onResolve={onResolveStarlightGift} />);
